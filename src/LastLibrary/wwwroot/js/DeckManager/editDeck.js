$(function () {
    ...
    //get the page JSON
    var deckJson = toCamel(JSON.parse($("#deckJson").html()));

    //set the card data to the global namespace
    deckJson.cards.forEach(function(cardData) {
        //set the multiverse ID as the key in the global namespace and set the card data as the object
        window.LASTLIBRARY.deckBuilder.cards[cardData.card.multiverseId.toString()] = cardData;
    });

    //set the deck name to the current deck's name
    $("#deckName").val(deckJson.deckName);

    //set the description
    $("#Description").val(deckJson.description);

    //set the isPublic checkbox
    if (deckJson.isPublic === true) $("#isPublic").prop("checked", true);

    //print the cards in the global namespace
...
/**
 * Function that converts Pascal Case objects to Camel Case
 * credit: http://stackoverflow.com/questions/12931828/convert-returned-json-object-properties-to-lower-first-camelcase
 */
function toCamel(o) {
